...
describe('classroom UI contracts', () => {
  ...
  it('shows immediate, persistent confirmation while a student answer is being saved', () => {
    const gamePage = readSource('../pages/GamePage.tsx')
    const styles = readSource('../styles.css')
    expect(gamePage).toContain('const selectedChoiceId = existingAnswer?.choiceId ?? savingChoiceId')
    expect(gamePage).toContain('const selectedCrisisChoiceId = existingCrisisAnswer?.choiceId ?? savingChoiceId')
    expect(gamePage).toContain('aria-pressed={selected}')
    expect(gamePage).toContain("{selected ? '✓' : index === 0 ? 'ก.' : 'ข.'}")
    expect(gamePage).toContain('รับคำตอบแล้ว กำลังยืนยัน…')
    expect(styles).toContain('.game-play-choice:not(:disabled):active')
    expect(styles).toContain('@keyframes student-choice-confirm')
    expect(styles).toContain('.student-choice-feedback')
  })
  ...
  it('finishes the grouped city reveal before ordinary progression or a manual Q4/Q8/Q10 checkpoint', () => {
    ...
    expect(teacherPage).toContain('text="1 ปีต่อมา..."')
    expect(teacherPage).toContain('text={`ปีที่ ${yearCutscene.cityYear}`}')
    expect(teacherPage).toContain('เมืองก้าวเข้าสู่ช่วงเวลาใหม่')
    expect(teacherPage).toContain("text={'การตัดสินใจของทุกคน\\nกำลังเปลี่ยนแปลงเมือง'}")
    expect(teacherPage).not.toContain('การตัดสินใจของประชาชน')
    expect(teacherPage.match(/<TypewriterText/g)).toHaveLength(4)
    expect(styles).toContain('.typewriter-text__measure')
    expect(styles).toContain('@keyframes typewriter-cursor-blink')
  ...
  it('keeps live impacts readable for 2.5 seconds without changing their trusted score', () => {
    ...
    expect(teacherPage).toContain('liveAnswerImpacts={liveAnswerImpacts}')
    expect(cityStage).toContain('<LiveAnswerImpacts impacts={liveAnswerImpacts} labelPositions={displayedLabelPositions} />')
    expect(cityStage).toContain('getDisplayedLabelPosition(building.id)')
    expect(liveImpacts).toContain('resolveLiveImpactAnchor(labelPosition)')
    expect(liveImpacts).toContain('item.locationId === impact.locationId')
    expect(liveImpacts).toContain('const IMPACT_LANES = [0, -1, 1]')
    expect(liveImpacts).toContain('{signedLocationScore(impact.score)}')
    expect(styles).toContain('.city-stage__workspace .city-stage__scene-overlays .live-answer-impacts')
  ...
  it('clears all temporary presentation state and timers at room boundaries', () => {
    ...
    expect(resetFlow).toContain('setBuildingLevelDisplayTransitions(null)')
  ...
  it('uses the completed layered city in Teacher View without exposing the composer', () => {
    ...
    expect(cityStage).toContain('buildingLevelTransitions={buildingLevelDisplayTransitions}')
    expect(cityStage).toContain('cityLevel={displayedCityLevel}')
    expect(cityScene).toContain('resolveCitySceneProfile')
    expect(cityScene).toContain('resolveBuildingVisualEffect(levelTransition)')
    expect(cityScene).toContain('city-scene__building')
    expect(cityScene).toContain('city-scene__building-environment')
    expect(cityScene).toContain('city-scene__building-ambient-glow')
    expect(cityScene).toContain('city-scene__building-ground-halo')
    expect(cityScene).toContain('is-effect-${visualEffect.state} is-intensity-${visualEffect.intensity}')
    expect(cityScene).not.toContain('city-scene__building-transition-aura')
    expect(cityScene).not.toContain('city-scene__building-aura')
  ...
  it('positions embedded teacher audio beside the menu with reserved tablet and portrait space', () => {
    const cityStage = readSource('./CityStage.tsx')
    const styles = readSource('../styles.css')
    expect(cityStage).toContain('<i aria-hidden="true">🗓️</i><span>ปีเมือง</span><strong>{room.currentQuestionNumber} / 10</strong>')
    expect(cityStage).not.toContain('<i aria-hidden="true">🚩</i><span>ปีเมือง</span>')
    expect(cityStage).not.toContain('<span>ข้อที่</span><strong>{room.currentQuestionNumber} / 10</strong>')
  ...
  it('uses one level-transition visual language for every building without score or building-specific effects', () => {
    const cityStage = readSource('./CityStage.tsx')
    const cityScene = readSource('./CityScene.tsx')
    const presentation = readSource('../domain/cityPresentation.ts')
    const teacherPage = readSource('../pages/TeacherPage.tsx')
    const styles = readSource('../styles.css')
    expect(presentation).toContain('export const resolveBuildingVisualEffect')
    expect(presentation).toContain('if (transition.currentLevel < 0)')
    expect(presentation).toContain("return { state: 'neutral', intensity: 'none' }")
    expect(presentation).toContain("state: 'upgrade'")
    expect(presentation).toContain("state: 'downgrade'")
    expect(cityScene).toContain('BUILDING_IDS.map((buildingId) =>')
    expect(cityScene).toContain('resolveBuildingVisualEffect(levelTransition)')
    expect(cityScene).toContain('data-building-visual-state={visualEffect.state}')
    expect(cityScene).toContain('data-building-effect-intensity={visualEffect.intensity}')
    expect(cityScene).not.toMatch(/scoreAverage|BuildingEffectTone|buildingEffects|buildingTransitions/)
    expect(cityScene).not.toMatch(/switch\s*\(buildingId\)|buildingId\s*===/)
    expect(cityStage).not.toMatch(/displayedBuildingEffects|city-stage__building-effects|city-stage__building-effect/)
    ...
    expect(styles).toContain('.city-scene__building-environment.is-upgrade .city-scene__building-ambient-glow')
    expect(styles).toContain('.city-scene__building-environment.is-downgrade .city-scene__building-ground-halo')
    expect(styles).toContain('.city-scene__building-warning-ring')
    expect(styles).toContain('mix-blend-mode: normal')
    expect(styles).toContain('.city-scene__building-environment.is-intensity-strong')
    expect(styles).toContain('.city-scene__building.is-effect-downgrade.is-intensity-strong')
    expect(styles).toContain('filter: blur(28px)')
    expect(styles).toContain('filter: blur(11px)')
    expect(styles).not.toContain('[data-building-id=')
    expect(styles).not.toContain('.city-stage__building-effect')
    expect(styles).not.toMatch(/has-(?:integrity|corruption)-effect/)
    expect(styles).not.toContain('.city-scene__building-aura')
    expect(styles).not.toContain('.city-scene__building-transition-aura')
  })
  ...
  it('renders compact navy level labels from the same visible model and transition state as CityScene', () => {
    ...
    expect(cityStage).toContain('buildingLevelDisplayTransitions?.[buildingId]')
    expect(cityStage).toContain('<span className="is-previous">Lv.{levelTransition.previousLevel}</span>')
    expect(cityStage).toContain('<span className="is-current">Lv.{levelTransition.currentLevel}</span>')
    expect(cityStage).toContain("levelTransition.changeDirection === 'same' ? (")
    expect(cityStage).toContain("levelTransition.changeDirection === 'up' ? '▲'")
    expect(cityStage).toContain(": '▼'")
    expect(cityStage).not.toContain("'→'")
    expect(teacherPage).toContain('resolveBuildingLevelDisplayTransitions(previousLevels, nextLevels)')
    expect(teacherPage).toContain('buildingLevelDisplayTransitions={buildingLevelDisplayTransitions ?? undefined}')
    ...
    expect(cityStage).toContain('resolveBuildingLabelTone(levelTransition.currentLevel)')
    expect(cityStage).toContain('resolveBuildingChangeIndicatorTone(levelTransition)')
    expect(cityStage).toContain('is-level-${labelTone} is-change-${levelTransition.changeDirection}')
    expect(styles).toContain('@property --building-label-flow-angle')
    expect(styles).toContain('city-building-label-border-flow')
    expect(styles).toContain('.city-stage__building-labels > button.is-level-critical')
    expect(styles).toContain('.city-stage__building-labels > button.is-level-degraded')
    expect(styles).toContain('.city-stage__building-labels > button.is-level-improved')
    expect(styles).toContain('.city-stage__building-labels > button.is-level-thriving')
    expect(styles).toContain('border-radius: 11px')
    expect(styles).toContain('background: linear-gradient(180deg, var(--building-label-bg-top), var(--building-label-bg-bottom))')
    expect(styles).toContain('.city-stage__building-label-level b.is-positive')
    expect(styles).toContain('.city-stage__building-label-level b.is-negative')
    expect(styles).toContain('.city-stage__building-label-level b.is-neutral')
    expect(styles).toContain('city-building-label-arrow-up')
    expect(styles).toContain('city-building-label-arrow-down')
    expect(styles).not.toContain('.city-stage__building-label-level b.is-same')
  })
  ...
  it('shows simplified building health plus cumulative current-cycle impact in the teacher right sidebar', () => {
    const cityStage = readSource('./CityStage.tsx')
    ...
    expect(cityStage).toContain('normalizeBuildingScores(')
    expect(cityStage).toContain('const buildingScore = displayedBuildingScores[buildingId]')
    expect(cityStage).toContain('const level = displayedBuildingLevels[buildingId]')
    expect(cityStage).toContain('BUILDING_LEVEL_STATUS_LABELS[level]')
    expect(cityStage).toContain('getBuildingScoreProgress(buildingScore)')
    expect(cityStage).toContain('สถานะอาคารทั้ง 7 แห่ง')
    expect(cityStage).toContain('หลอดแสดงสุขภาพอาคาร • ตัวเลขคือผลสะสม')
    expect(cityStage).toContain('<small>{levelLabel}</small>')
    expect(cityStage).toContain('<small>ผลสะสม</small>')
    expect(cityStage).toContain("title=\"ผลกระทบสะสมรอบนี้\"")
    expect(cityStage).toContain('className="city-stage__building-health"')
    expect(cityStage).not.toContain("toLocaleString('th-TH')} / 1,000")
    expect(cityStage).not.toContain("ระดับ {level > 0 ? '+' : ''}{level}")
