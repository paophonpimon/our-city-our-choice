...
describe('classroom UI contracts', () => {
  ...
  it('provides an opt-in central Draft/Published scene calibration tool backed by the real renderer', () => {
    const app = readSource('../App.tsx')
    const cityStage = readSource('./CityStage.tsx')
    const cityScene = readSource('./CityScene.tsx')
    const layoutEditorPage = readSource('../pages/LayoutEditorPage.tsx')
    const cityLayoutOverrides = readSource('../domain/cityLayoutOverrides.ts')
    const layoutContext = readSource('../context/CityLayoutContext.tsx')
    const layoutManager = readSource('../hooks/useCityLayoutManager.ts')
    const packageJson = readSource('../../package.json')
    expect(cityStage).toContain("get('layout') === '1'")
    expect(app).toContain('path="/layout-editor"')
    expect(layoutEditorPage).toContain('layoutMode')
    expect(layoutEditorPage).toContain("onExitLayoutMode={() => navigate('/teacher')}")
    expect(packageJson).toContain('"dev:staging"')
    expect(cityStage).toContain('useCityLayoutManager(isLayoutMode)')
    ...
    expect(cityScene).toContain('resolveProductionPlacement(publishedLayout, sceneProfile.id, buildingId, levels[buildingId])')
    expect(cityScene).not.toContain('localStorage')
    expect(layoutContext).toContain('service.subscribePublishedCityLayout(')
    expect(layoutManager).toContain('service.saveCityLayoutDraft(uid, records)')
    expect(layoutManager).toContain('window.setTimeout(() => { void flushPending() }, 180)')
    expect(layoutManager).toContain('countCityLayoutDraftPlacements(draft) > 0')
    expect(layoutManager).toContain('countCityLayoutDraftLabels(labelDraft) > 0')
    expect(layoutManager).toContain('setLegacyOfferChecked(true)')
    expect(layoutManager).toContain('if (hasLegacy) setLegacyDraft(readCityLayoutOverrides())')
    expect(cityStage).not.toContain('CITY_LAYOUT_STORAGE_KEY')
    expect(cityStage).toContain('ใช้ตำแหน่งนี้กับทั้ง 5 ระดับ')
    expect(cityStage).toContain('ใช้ตำแหน่งนี้กับทั้ง 3 ฉาก')
    expect(cityStage).toContain('ตรวจสอบก่อนเผยแพร่')
    expect(cityStage).toContain('STAGING LAYOUT EDITOR')
    expect(cityStage).toContain('Production ปิดการแก้ไข Layout')
    expect(cityStage).not.toContain('ยังไม่ได้กำหนดผู้ดูแล Layout')
    expect(layoutContext).toContain("service.cityLayoutRuntime !== 'staging'")
  ...
  it('renders compact navy level labels from the same visible model and transition state as CityScene', () => {
    const cityStage = readSource('./CityStage.tsx')
    const styles = readSource('../styles.css')
    expect(cityStage).toContain('const displayedLevel = displayedBuildingLevels[buildingId]')
    expect(cityStage).toContain('if (buildingTransitions?.[buildingId])')
    expect(cityStage).toContain('from: previousDisplayedBuildingLevelsRef.current[buildingId]')
    expect(cityStage).toContain('to: displayedBuildingLevels[buildingId]')
    expect(cityStage).toContain('<span>Lv.{displayedLevel}</span>')
    expect(cityStage).toContain('<b>▸</b>')
    expect(cityStage).not.toContain('<i>{building.icon}</i>{building.label}')
    expect(cityStage).not.toContain('aria-hidden="true">ดูข้อมูล</small>')
    expect(styles).toContain('border: 2px solid rgb(39 202 245 / 92%)')
    expect(styles).toContain('border-radius: 11px')
    expect(styles).toContain('background: linear-gradient(180deg, rgb(5 30 55 / 94%), rgb(2 16 34 / 93%))')
    expect(styles).toContain('.city-stage__building-label-level .is-negative')
    expect(styles).toContain('.city-stage__building-label-level .is-positive')
  })

  it('calibrates labels separately through central Draft without changing model controls', () => {
    const cityStage = readSource('./CityStage.tsx')
    const layoutManager = readSource('../hooks/useCityLayoutManager.ts')
    expect(cityStage).toContain("type LayoutCalibrationTarget = 'model' | 'label'")
    expect(cityStage).toContain('ปรับโมเดล')
    expect(cityStage).toContain('ปรับป้ายชื่อ')
    expect(cityStage).toContain('startLabelLayoutDrag(event, buildingId)')
    expect(cityStage).toContain('updateLabelLayoutPlacement(drag.buildingId, {')
    expect(cityStage).not.toContain('updateLayoutPlacement(drag.buildingId, {\n      labelX:')
    expect(cityStage).toContain('Label X')
    expect(cityStage).toContain('Label Y')
    expect(cityStage).toContain('ใช้ตำแหน่งป้ายนี้กับทั้ง 5 ระดับ')
    expect(cityStage).toContain('ใช้ตำแหน่งป้ายนี้กับทั้ง 3 ฉาก')
    expect(cityStage).toContain('ใช้กับทุกระดับและทุกฉาก')
    expect(cityStage).toContain('คืนตำแหน่งป้ายจุดนี้')
    expect(cityStage).toContain('cityLayoutLabelDraftId(layoutSceneId, buildingId, layoutModelLevel)')
    expect(layoutManager).toContain('service.saveCityLayoutDraft(uid, records)')
    expect(layoutManager).toContain('resolveCompleteCityLayout(draft, publishedLayout, labelDraft)')
  })

  it('shows cumulative current-cycle building impact in the teacher right sidebar', () => {
    const cityStage = readSource('./CityStage.tsx')
    expect(cityStage).toContain('calculateCumulativeBuildingImpact({')
    expect(cityStage).toContain('currentGameCycle: room.gameCycle')
    expect(cityStage).toContain('currentQuestionNumber: room.currentQuestionNumber')
    expect(cityStage).toContain('const score = cumulativeBuildingImpacts[building.id]')
    expect(cityStage).toContain("const tone = score > 0 ? 'is-positive' : score < 0 ? 'is-negative' : 'is-neutral'")
